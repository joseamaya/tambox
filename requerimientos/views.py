...
class ListadoRequerimientos(ListView):
    ...
    def get_queryset(self):
        ...
        if oficina_usuario == logistica and puesto_usuario.es_jefatura:
            queryset = Requerimiento.objects.all().exclude(estado=Requerimiento.STATUS.CANC).order_by('codigo')
        else:
            queryset = Requerimiento.objects.filter(solicitante__usuario=self.request.user).exclude(
                estado=Requerimiento.STATUS.CANC).order_by('codigo')
        return queryset
...
class TransferenciaRequerimiento(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        requerimientos = Requerimiento.objects.filter(
            aprobacionrequerimiento__estado=AprobacionRequerimiento.STATUS.APROB_PRES).filter(
            Q(estado=Requerimiento.STATUS.PEND) | Q(estado=Requerimiento.STATUS.COTIZ_PARC) | Q(
...
class ReportePDFRequerimiento(View):
    ...
    def detalle(self, pdf, y, requerimiento):
        ...
        for detalle in detalles:
            try:
                tupla_producto = (detalle.nro_detalle, detalle.cantidad, detalle.producto.unidad_medida.descripcion,
                                  detalle.producto.descripcion, detalle.uso)
        ...
        tabla_detalle = Table([encabezados] + lista_detalles + adicionales,
                              colWidths=[0.7 * cm, 1.8 * cm, 1.5 * cm, 7.5 * cm, 8 * cm])
    ...
    def firmas(self, pdf, y, requerimiento):
        ...
        pie = [(u'Fecha :     /     /   ', 'Solicitado por:', 'Jefe de Departamento', u'V° B° Gerente Inmediato',
                u'Vº Bº Gerente Adm.', u'Vº Bº Presupuesto')]
        tabla_observaciones = Table(encabezados + cuerpo + pie,
                                    colWidths=[2.8 * cm, 3.1 * cm, 3.4 * cm, 3.4 * cm, 3.4 * cm, 3.4 * cm],
                                    rowHeights=[0.5 * cm, 2 * cm, 0.5 * cm])
        tabla_observaciones.setStyle(TableStyle(
            [
