...
class NavigableQuerySet(models.query.QuerySet):
    ...
    consumidores que necesitan la instance: el tag `previous_url` llama a
    ...
    def previous(self, instance):
        try:
            return self.filter(pk__lt=instance.pk).order_by('-pk')[0]
        except IndexError:
            return self.order_by('pk').last()

    def next(self, instance):
        try:
            return self.filter(pk__gt=instance.pk).order_by('pk')[0]
        except IndexError:
