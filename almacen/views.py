...
class ReporteConsolidadoProductosKardexExcel(FormView):
    ...
    def obtener_kardex(self, producto, almacen, anio, mes):
        listado_kardex = Kardex.objects.filter(almacen__codigo=almacen,
        ...
        if len(listado_kardex) > 0:
            ...
            t_valor_i = 0
            t_valor_s = 0
        return (t_cantidad_i, t_valor_i, t_cantidad_s, t_valor_s)

    def post(self, request, *args, **kwargs):
        mes = request.POST['meses']
        anio = request.POST['anios']
        almacen = request.POST['almacenes']
        productos = ControlProductoAlmacen.objects.filter(almacen__codigo=almacen)
        wb = Workbook()
        ws = wb.active
        ws['B1'] = u'Almacén: ' + almacen
        ws['E1'] = 'Periodo: ' + mes + '-' + anio
        ...
        for control_producto in productos:
            ...
            mes_ant, anio_ant = self.obtener_mes_anterior(mes, anio)
            cantidad_ingreso, valor_ingreso, cantidad_salida, valor_salida = self.obtener_kardex(producto, almacen,
                                                                                                 anio_ant, mes_ant)
            cant_saldo_inicial = cantidad_ingreso - cantidad_salida
            valor_saldo_inicial = valor_ingreso - valor_salida
...
class GeneracionKardexProducto(FormView):
    ...
    def detalle_permanente_valorizado(self, pdf, x, y, mes, anio, almacen, producto):
        encabezados = (
        'FECHA', 'TIPO (TABLA 10)', 'SERIE', 'NÚMERO', u'TIPO DE OPERACIÓN ', 'CANTIDAD', 'COSTO UNIT.', 'COSTO TOT.',
