...
from almacen.forms import AlmacenForm, TipoStockForm, TipoSalidaForm, TipoMovimientoForm, FormularioReporteMovimientos, \
    FormularioKardexProducto, CargarInventarioInicialForm, FormularioAprobacionPedido, FormularioReporteStock, \
    MovimientoForm, \
...
class AprobarPedido(FormView):
    ...
    def guardar_cabecera_movimiento(self, tipo_salida, fecha_operacion, almacen, usuario, total, referencia, oficina,
                                    observaciones):
    ...
    def guardar_detalle_con_referencia(self, cdetalles, referencia, movimiento):
        ...
        for i in range(cdetalles):
            try:
                ...
                self.guardar_detalle_movimiento(cont_detalles, detalle_pedido, movimiento, producto, cantidad, precio,
                                                valor)
    ...
    def guardar_aprobacion(self, codigo, r_almacen, cdetalles, r_fecha, r_hora, total, observaciones):
        ...
        with transaction.atomic():
            movimiento = self.guardar_cabecera_movimiento(tipo_salida, fecha_operacion, almacen, usuario, total,
                                                          referencia, oficina, observaciones)
...
class BusquedaProductosAlmacen(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            control_productos = ControlProductoAlmacen.objects.filter(almacen__codigo=almacen,
                                                                      producto__descripcion__icontains=descripcion)[:20]
            lista_productos = []
            for control in control_productos:
                producto_json = {'label': control.producto.descripcion, 'codigo': control.producto.codigo,
                                 'descripcion': control.producto.descripcion,
                                 'unidad': control.producto.unidad_medida.descripcion, 'precio': str(control.precio)}
...
class CrearDetalleSalida(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            lista_detalles = []
            det = {'codigo': '', 'nombre': '', 'cantidad': '0', 'precio': '0', 'unidad': '', 'valor': '0'}
            lista_detalles.append(det)
            formset = DetalleSalidaFormSet(initial=lista_detalles)
            lista_json = []
            for form in formset:
                detalle_json = {'codigo': str(form['codigo']), 'nombre': str(form['nombre']),
                                'cantidad': str(form['cantidad']), 'precio': str(form['precio']),
                                'unidad': str(form['unidad']), 'valor': str(form['valor'])}
...
class CrearDetallePedido(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            lista_detalles = []
            det = {'codigo': '', 'nombre': '', 'cantidad': '0', 'unidad': ''}
            lista_detalles.append(det)
            formset = DetallePedidoFormSet(initial=lista_detalles)
            lista_json = []
            for form in formset:
                detalle_json = {'codigo': str(form['codigo']), 'nombre': str(form['nombre']),
                                'cantidad': str(form['cantidad']), 'unidad': str(form['unidad'])}
                lista_json.append(detalle_json)
            data = json.dumps(lista_json)
            return HttpResponse(data, 'application/json')
...
class CrearDetalleIngreso(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            lista_detalles = []
            det = {'orden_compra': '0', 'codigo': '', 'nombre': '', 'cantidad': '0', 'precio': '0', 'unidad': '',
                   'valor': '0'}
            lista_detalles.append(det)
            formset = DetalleIngresoFormSet(initial=lista_detalles)
            lista_json = []
            for form in formset:
                detalle_json = {'orden_compra': str(form['orden_compra']), 'codigo': str(form['codigo']),
                                'nombre': str(form['nombre']), 'cantidad': str(form['cantidad']),
                                'precio': str(form['precio']), 'unidad': str(form['unidad']),
                                'valor': str(form['valor'])}
                lista_json.append(detalle_json)
            data = json.dumps(lista_json)
            return HttpResponse(data, 'application/json')
            ...
            with transaction.atomic():
                ...
                for detalle_pedido_form in detalle_pedido_formset:
                    ...
                    if codigo and cantidad:
                        ...
                        detalles.append(
                            DetallePedido(pedido=self.object, nro_detalle=cont, producto=producto, cantidad=cantidad))
                        cont += 1
...
class EliminarAlmacen(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            almacen_json = {'codigo': almacen.codigo, 'descripcion': almacen.descripcion}
    ...
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            for kardex in detalle_kardex:
                ...
                kardex.delete()
                kardex.delete()
            Movimiento.objects.filter(id_movimiento=id_movimiento).update(estado=Movimiento.STATUS.CANC,
                                                                          referencia=None)
...
class ReporteConsolidadoGruposKardexExcel(FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for grupo in grupos:
            ...
            listado_kardex_ant = Kardex.objects.filter(almacen__codigo=almacen, fecha_operacion__year=anio_ant,
                                                       fecha_operacion__month=mes_ant,
                                                       producto__grupo_productos=grupo).order_by(
                'producto__descripcion', 'fecha_operacion', 'cantidad_salida', 'created')
            ...
            listado_kardex = Kardex.objects.filter(almacen__codigo=almacen, fecha_operacion__year=anio,
...
class ReporteConsolidadoProductosKardexExcel(FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for producto in productos:
            ...
            mes_ant, anio_ant = self.obtener_mes_anterior(mes, anio)
            listado_kardex_ant = Kardex.objects.filter(almacen__codigo=almacen, fecha_operacion__year=anio_ant,
                                                       fecha_operacion__month=mes_ant, producto=producto).order_by(
                'producto__descripcion', 'fecha_operacion', 'cantidad_salida', 'created')
            if len(listado_kardex_ant) > 0:
                c_s_i = listado_kardex_ant.aggregate(Sum('cantidad_total'))
                cant_saldo_inicial = c_s_i['cantidad_total__sum']
            ...
            ws.cell(row=cont, column=5).number_format = '#.00000'
            listado_kardex = Kardex.objects.filter(almacen__codigo=almacen, fecha_operacion__year=anio,
                                                   fecha_operacion__month=mes, producto=producto).order_by(
                'producto__descripcion', 'fecha_operacion', 'cantidad_salida', 'created')
            if len(listado_kardex) > 0:
                cantidad_ingreso = listado_kardex.aggregate(Sum('cantidad_ingreso'))
                cantidad_salida = listado_kardex.aggregate(Sum('cantidad_salida'))
...
class GeneracionKardexProducto(FormView):
    ...
    def obtener_formato_normal(self, cod_prod, producto, mes, anio, almacen):
        ...
        mes_ant, anio_ant = self.obtener_mes_anterior(mes, anio)
        listado_kardex_ant = Kardex.objects.filter(almacen__codigo=almacen, fecha_operacion__year=anio_ant,
                                                   fecha_operacion__month=mes_ant, producto=producto.codigo).order_by(
            'producto__descripcion', 'fecha_operacion', 'cantidad_salida', 'created')
        if len(listado_kardex_ant) > 0:
            c_s_i = listado_kardex_ant.aggregate(Sum('cantidad_total'))
            cant_saldo_inicial = c_s_i['cantidad_total__sum']
        ...
        cont += 1
        listado_kardex = Kardex.objects.filter(almacen=almacen, fecha_operacion__year=anio, fecha_operacion__month=mes,
                                               producto=producto).order_by('producto__descripcion', 'fecha_operacion',
    ...
    def cabecera(self, pdf, x, y, mes, anio, almacen, producto, valorizado):
        ...
        pdf.drawString(x, y - 120,
                       u"TIPO (TABLA 5): " + producto.tipo_existencia.codigo_sunat + " - " + producto.tipo_existencia.descripcion)
        pdf.drawString(x, y - 140, u"DESCRIPCIÓN: " + producto.descripcion)
        pdf.drawString(x, y - 160,
                       u"CÓDIGO DE LA UNIDAD DE MEDIDA (TABLA 6): " + producto.unidad_medida.codigo + " - " + producto.unidad_medida.descripcion)
        if valorizado:
    ...
    def detalle_permanente_unidades_fisicas(self, pdf, y, mes, anio, almacen, producto):
        encabezados = (
            'FECHA', 'TIPO (TABLA 10)', 'SERIE', 'NÚMERO', u'TIPO DE OPERACIÓN ', 'ENTRADAS', 'SALIDAS', 'SALDO  FINAL')
        mes_ant, anio_ant = self.obtener_mes_anterior(mes, anio)
        listado_kardex_ant = Kardex.objects.filter(almacen=almacen, fecha_operacion__year=anio_ant,
                                                   fecha_operacion__month=mes_ant, producto=producto).order_by(
            'producto__descripcion', 'fecha_operacion', 'cantidad_salida', 'created')
        if len(listado_kardex_ant) > 0:
            c_s_i = listado_kardex_ant.aggregate(Sum('cantidad_total'))
        ...
        if len(listado_kardex) > 0:
            ...
            t_cantidad_s = cantidad_salida['cantidad_salida__sum']
            t_cantidad_t = cantidad_total['cantidad_total__sum']
            detalles = [(kardex.fecha_operacion.strftime('%d/%m/%Y'), kardex.movimiento.tipo_documento.codigo_sunat,
                         kardex.movimiento.serie, kardex.movimiento.numero,
                         kardex.movimiento.tipo_movimiento.codigo_sunat, kardex.cantidad_ingreso,
                         kardex.cantidad_salida, kardex.cantidad_total) for kardex in listado_kardex]
        ...
        for lista in listas:
            cont_listas += 1
            detalle_orden = Table([encabezados] + lista,
                                  colWidths=[2.5 * cm, 3.5 * cm, 3 * cm, 3 * cm, 3.5 * cm, 3.5 * cm, 3.5 * cm])
    ...
    def detalle_permanente_valorizado(self, pdf, x, y, mes, anio, almacen, producto):
        encabezados = (
            'FECHA', 'TIPO (TABLA 10)', 'SERIE', 'NÚMERO', u'TIPO DE OPERACIÓN ', 'CANTIDAD', 'COSTO UNIT.',
            'COSTO TOT.',
            'CANTIDAD', 'COSTO UNIT.', 'COSTO TOT.', 'CANTIDAD', 'COSTO UNIT.', 'COSTO TOT.')
        mes_ant, anio_ant = self.obtener_mes_anterior(mes, anio)
        listado_kardex_ant = Kardex.objects.filter(almacen=almacen, fecha_operacion__year=anio_ant,
                                                   fecha_operacion__month=mes_ant, producto=producto).order_by(
            'producto__descripcion', 'fecha_operacion', 'cantidad_salida', 'created')
        ...
        if len(listado_kardex) > 0:
            ...
            t_valor_s = valor_salida['valor_salida__sum']
            t_valor_t = valor_total['valor_total__sum']
            detalles = [(kardex.fecha_operacion.strftime('%d/%m/%Y'), '', '', '',
                         kardex.movimiento.tipo_movimiento.codigo_sunat, kardex.cantidad_ingreso, kardex.precio_ingreso,
                         kardex.valor_ingreso, kardex.cantidad_salida, kardex.precio_salida, kardex.valor_salida,
                         kardex.cantidad_total, kardex.precio_total, kardex.valor_total) for kardex in listado_kardex]
        try:
            precio_saldo_inicial = valor_saldo_inicial / cant_saldo_inicial
        except:
            precio_saldo_inicial = 0
        detalles.insert(0, (
            '01/' + mes + "/" + anio, '00', 'SALDO', 'INICIAL', '16', 0, 0, 0, 0, 0, 0, cant_saldo_inicial,
            precio_saldo_inicial, valor_saldo_inicial))
        try:
        ...
        try:
            ...
            t_precio_t = 0
        detalles.append(('', '', '', '', 'TOTALES', t_cantidad_i, t_precio_i, t_valor_i, t_cantidad_s, t_precio_s,
                         t_valor_s, t_cantidad_t, t_precio_t, t_valor_t))
        size = 30
        listas = [detalles[i:i + size] for i in range(0, len(detalles), size)]
        cant_listas = len(listas)
        cont_listas = 0
        ...
        for lista in listas:
            cont_listas += 1
            detalle_orden = Table([encabezados] + lista,
                                  colWidths=[1.7 * cm, 2.5 * cm, 1.3 * cm, 1.5 * cm, 3.2 * cm, 1.8 * cm, 2 * cm, 2 * cm,
                                             1.8 * cm, 2 * cm, 2 * cm, 1.8 * cm, 2 * cm, 2 * cm])
            detalle_orden.setStyle(TableStyle(
                [
                    ('GRID', (0, 0), (-1, -1), 1, colors.black),
                    ('FONTSIZE', (0, 0), (-1, -1), 8),
...
class ReporteExcelKardex(FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for prod in productos:
            ...
            listado_kardex_ant = Kardex.objects.filter(almacen__codigo=almacen, fecha_operacion__year=anio_ant,
                                                       fecha_operacion__month=mes_ant,
                                                       producto=producto.codigo).order_by('producto__descripcion',
                                                                                          'fecha_operacion',
                                                                                          'cantidad_salida', 'created')
            if len(listado_kardex_ant) > 0:
                c_s_i = listado_kardex_ant.aggregate(Sum('cantidad_total'))
            ...
            cont += 1
            listado_kardex = Kardex.objects.filter(almacen__codigo=almacen, fecha_operacion__year=anio,
                                                   fecha_operacion__month=mes, producto=producto.codigo).order_by(
                'producto__descripcion', 'fecha_operacion', 'cantidad_salida', 'created')
            if len(listado_kardex) > 0:
                cantidad_ingreso = listado_kardex.aggregate(Sum('cantidad_ingreso'))
                cantidad_salida = listado_kardex.aggregate(Sum('cantidad_salida'))
...
class ReporteExcelMovimientos(FormView):
    ...
    def form_valid(self, form):
        ...
        if tipo_busqueda == 'F':
            ...
            ws['D3'] = 'AÑO'
            ws['E3'] = annio
            movimientos = Movimiento.objects.filter(fecha_operacion__month=mes, fecha_operacion__year=annio,
                                                    tipo_movimiento=tipo_movimiento, almacen=almacen)
            ...
            ws['B3'] = 'AÑO'
            ws['C3'] = annio
            movimientos = Movimiento.objects.filter(fecha_operacion__year=annio, tipo_movimiento=tipo_movimiento,
                                                    almacen=almacen)
        ws['B5'] = 'ID_MOVIMIENTO'
...
class ReportePDFMovimiento(View):
    ...
    def detalle(self, pdf, y, movimiento):
        ...
        detalles = [(detalle.nro_detalle, detalle.cantidad, detalle.producto.unidad_medida.codigo,
                     detalle.producto.descripcion, detalle.precio, detalle.valor) for detalle in
                    DetalleMovimiento.objects.filter(movimiento=movimiento)]
        size = 30
        listas = [detalles[i:i + size] for i in range(0, len(detalles), size)]
        cant_listas = len(listas)
        cont_listas = 0
        ...
        for lista in listas:
            cont_listas += 1
            detalle_orden = Table([encabezados] + lista,
                                  colWidths=[1.5 * cm, 2.5 * cm, 1.5 * cm, 8.5 * cm, 2 * cm, 2.5 * cm])
            detalle_orden.setStyle(TableStyle(
                [
                    ('GRID', (0, 0), (-1, -1), 1, colors.black),
                    ('FONTSIZE', (0, 0), (-1, -1), 8),
...
class VerificarStockParaPedido(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        detalles = DetallePedido.objects.filter(pedido__codigo=pedido, estado=DetallePedido.STATUS.PEND).order_by(
            'nro_detalle')
        lista_detalles = []
        for detalle in detalles:
            try:
                control_producto = ControlProductoAlmacen.objects.get(almacen__codigo=almacen,
                ...
                precio = 0
            if stock != 0:
                detalle_json = {'codigo_detalle': detalle.id, 'codigo_producto': detalle.producto.codigo,
                                'nombre_producto': detalle.producto.descripcion,
                                'unidad_producto': detalle.producto.unidad_medida.descripcion}
