...
class EliminarFormaPago(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            forma_pago_json = {'codigo': forma_pago.codigo, 'descripcion': forma_pago.descripcion}
...
class EliminarTipoDocumento(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            tipo_documento_json = {'codigo_sunat': tipo_documento.codigo_sunat, 'nombre': tipo_documento.nombre}
...
class ModificarTipoDocumento(UpdateView):
    ...
    @method_decorator(
        permission_required('contabilidad.change_tipo_documento', reverse_lazy('seguridad:permiso_denegado')))
    def dispatch(self, *args, **kwargs):
