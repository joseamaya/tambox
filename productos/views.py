    ...
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            for producto in productos:
                producto_json = {'label': producto.descripcion, 'codigo': producto.codigo,
                                 'descripcion': producto.descripcion, 'unidad': producto.unidad_medida.descripcion,
...
class BusquedaProductosCodigo(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            lista_productos = []
            for producto in productos:
                producto_json = {'label': producto.codigo, 'codigo': producto.codigo,
                                 'descripcion': producto.descripcion, 'unidad': producto.unidad_medida.descripcion,
                                 'precio': str(producto.precio)}
                lista_productos.append(producto_json)
            data = json.dumps(lista_productos)
    ...
    def form_valid(self, form):
        ...
        for fila in data_reader:
            try:
                ...
                descripcion = fila[1]
                grupo_productos, creado = GrupoProductos.objects.get_or_create(
                    descripcion=unicode(descripcion, errors='ignore'),
            ...
            try:
                producto, creado = Producto.objects.get_or_create(descripcion=unicode(fila[1].strip(), errors='ignore'),
                                                                  defaults={'unidad_medida': und,
                                                                            ...
                                                                            'desc_abreviada': unicode(fila[4].strip(),
                                                                                                      errors='ignore')})
            except:
                pass
...
class EliminarGrupoProductos(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            grupo_productos_json = {'codigo': grupo_productos.codigo, 'descripcion': grupo_productos.descripcion}
...
class EliminarProducto(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            producto_json = {'codigo': producto.codigo, 'descripcion': producto.descripcion}
