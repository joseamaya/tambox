    ...
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            for proveedor in proveedores:
                ...
                unidad_json['direccion'] = proveedor.direccion
    ...
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            if len(orden.conformidadservicio_set.all()) > 0:
                conformidad_json['conformidades'] = 'SI'
            else:
                conformidad_json['conformidades'] = 'NO'
                with transaction.atomic():
                    if orden.cotizacion is not None:
                        orden.eliminar_referencia()
                    OrdenServicios.objects.filter(codigo=codigo).update(estado=OrdenServicios.STATUS.CANC,
                                                                        cotizacion=None)
    ...
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            if tipo_busqueda == 'PRODUCTOS':
                ...
                try:
                    monto_impuesto = CONFIGURACION.monto
                except:
                    monto_impuesto = 0
...
class ReportePDFOrdenCompra(View):
    ...
    def detalle(self, pdf, y, orden):
        ...
        try:
            ...
            detalles = [(detalle.nro_detalle, detalle.cantidad, detalle.producto.unidad_medida.descripcion,
                         detalle.producto.descripcion, detalle.precio, round(detalle.valor, 5)) for detalle in
    ...
    def cuadro_total(self, pdf, y, orden):
        ...
        total = [[round(orden.subtotal, 2)], [round(orden.igv, 2)], [round(orden.total, 2)]]
...
class ReportePDFMemorandoConformidadServicio(View):
    def cabecera(self, pdf, conformidad):
        ...
        puesto_gerente = Puesto.objects.get(oficina=gerencia_inmediata, es_jefatura=True, estado=True)
        ...
        puesto_solicitante = puestos[0]
        puesto_jefe_inmediato = puesto_solicitante.puesto_superior
        jefe_inmediato = puesto_jefe_inmediato.trabajador
        y = 690
        if puesto_solicitante.oficina.codigo == 'GGEN':
            puesto_gerente = Puesto.objects.get(oficina=CONFIGURACION.administracion)
            gerente = puesto_gerente.trabajador
        if puesto_gerente.pk == puesto_jefe_inmediato.pk or puesto_jefe_inmediato.pk == puesto_solicitante.pk:
            ...
            y = y - 20
            pdf.drawString(50, y, u"DE        :     " + solicitante.nombre_completo())
            y = y - 20
