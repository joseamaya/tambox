...
class BusquedaCotizacion(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            cotizacion_json = {'ruc': cotizacion.proveedor.ruc, 'razon_social': cotizacion.proveedor.razon_social,
    ...
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            for proveedor in proveedores:
                unidad_json = {'label': proveedor.razon_social, 'ruc': proveedor.ruc}
...
class BusquedaProveedoresRUC(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            proveedor_json = {'razon_social': proveedor.razon_social, 'direccion': proveedor.direccion,
                              'estado': proveedor.estado_sunat}
...
class ObtenerDetalleCotizacion(TemplateView):
    def get(self, request, *args, **kwargs):
        if request.is_ajax():
            ...
            for form in formset:
                detalle_json = {'cotizacion': str(form['cotizacion']), 'codigo': str(form['codigo']),
                                'nombre': str(form['nombre']), 'precio': str(form['precio']),
        ...
        if request.is_ajax():
            ...
            for detalle in detalles:
                det = {'orden_servicios': detalle.id,
                       'servicio': detalle.detalle_cotizacion.detalle_requerimiento.producto.descripcion,
                       'uso': detalle.detalle_cotizacion.detalle_requerimiento.uso, 'precio': str(detalle.precio),
                       'cantidad': str(detalle.cantidad), 'valor': str(detalle.valor)}
                lista_detalles.append(det)
            formset = DetalleConformidadServicioFormSet(initial=lista_detalles)
            lista_json = []
            for form in formset:
                detalle_json = {'orden_servicios': str(form['orden_servicios']), 'servicio': str(form['servicio']),
                                'uso': str(form['uso']), 'precio': str(form['precio']),
                                'cantidad': str(form['cantidad']), 'valor': str(form['valor'])}
                lista_json.append(detalle_json)
            data = json.dumps(lista_json)
            return HttpResponse(data, 'application/json')
    ...
    def detalle(self, pdf, y, orden):
        ...
        try:
            detalles = [(detalle.nro_detalle, detalle.cantidad,
                         ...
                         detalle.valor) for detalle in DetalleOrdenCompra.objects.filter(orden=orden)]
        except:
...
class TransferenciaOrdenServicios(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['ordenes'] = OrdenServicios.objects.filter(cotizacion__requerimiento__solicitante=trabajador,
